...
describe("findQuarterlyEntryIn10K", () => {
  test("finds quarterly entry with ~90 day span in 10-K", () => {
    ...
    const entries = [
      ...
      entry({ start: "2023-10-01", end: "2024-09-28", val: 400000000000, fp: "FY" }),
      // Q4 quarterly entry (~91 days)
      entry({ start: "2024-06-30", end: "2024-09-28", val: 102500000000, fp: "Q4" }),
    ];
  ...
  test("ignores entries with wrong date", () => {
    ...
    const entries = [
      ...
      entry({ start: "2024-01-01", end: "2024-03-30", val: 90000000000, fp: "Q2", form: "10-K" }),
    ];
    const periodEnd = new Date(2024, 8, 28);
    const result = client.findQuarterlyEntryIn10K(entries, periodEnd);
    expect(result).toBeNull();
  ...
  test("ignores 10-Q entries", () => {
    const client = new EdgarClient(makeCache() as never);
    const entries = [
      entry({ start: "2024-06-30", end: "2024-09-28", val: 102500000000, fp: "Q4", form: "10-Q" }),
    ];
    const periodEnd = new Date(2024, 8, 28);
    const result = client.findQuarterlyEntryIn10K(entries, periodEnd);
    expect(result).toBeNull();
...
describe("findAnnualEntryIn10K", () => {
  test("finds entry with fp=FY", () => {
    const client = new EdgarClient(makeCache() as never);
    const entries = [
      entry({ start: "2023-10-01", end: "2024-09-28", val: 416200000000, fp: "FY" }),
      entry({ start: "2024-06-30", end: "2024-09-28", val: 102500000000, fp: "Q4" }),
    ];
    const periodEnd = new Date(2024, 8, 28);
  ...
  test("finds entry with >350 day span", () => {
    const client = new EdgarClient(makeCache() as never);
    const entries = [
      entry({ start: "2023-10-01", end: "2024-09-28", val: 416200000000, fp: "Q4" }), // FP mislabeled but span is annual
    ];
    const periodEnd = new Date(2024, 8, 28);
...
describe("computeQ4ByDecomposition", () => {
  test("correctly decomposes Q4 = annual - Q1 - Q2 - Q3", async () => {
    ...
    const mockEntries = [
      ...
      entry({ start: "2023-10-01", end: "2024-09-28", val: 416200000000, fp: "FY", form: "10-K" }),
      // Q1 (10-Q, Dec 30 2023)
      entry({ start: "2023-10-01", end: "2023-12-30", val: 119600000000, fp: "Q1", form: "10-Q" }),
      // Q2 (10-Q, Mar 30 2024)
      entry({ start: "2024-01-01", end: "2024-03-30", val: 94900000000, fp: "Q2", form: "10-Q" }),
      // Q3 (10-Q, Jun 29 2024)
      entry({ start: "2024-04-01", end: "2024-06-29", val: 99200000000, fp: "Q3", form: "10-Q" }),
    ];

    cache.set("edgar_concepts", `AAPL-${concept}`, mockEntries);

    const result = await client.computeQ4ByDecomposition("AAPL", 2024, revenueMetric);
    expect(result).toBe(416200000000 - 119600000000 - 94900000000 - 99200000000);
    expect(result).toBe(102500000000);
  });
  ...
  test("returns null for EPS (non-additive)", async () => {
    ...
    const result = await client.computeQ4ByDecomposition("AAPL", 2024, epsMetric);
    expect(result).toBeNull();
  });
  ...
  test("returns null for shares_outstanding (non-additive)", async () => {
    ...
    const result = await client.computeQ4ByDecomposition("AAPL", 2024, sharesMetric);
    expect(result).toBeNull();
  });
