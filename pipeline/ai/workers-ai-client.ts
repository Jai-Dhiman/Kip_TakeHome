...
export class WorkersAIClient implements LLMClient {
	...
	async chatJSON<T>(
		...
		const text = await this.callAPI(messages, maxTokens, {
			type: "json_object",
		});
	...
	private async callResponsesAPI(
		...
		if (!res.ok) {
			...
			throw new Error(
				`Workers AI Responses API request failed (${res.status}): ${errText}`,
			);
		}
...
const KNOWN_FIELDS = [
	...
	"most_concerning_finding",
	"strongest_transparency_evidence",
];
