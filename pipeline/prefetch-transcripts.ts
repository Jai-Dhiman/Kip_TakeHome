...
if (API_KEYS.length === 0) {
	console.error(
		"Usage: bun run pipeline/prefetch-transcripts.ts <key1> [key2] [key3...]",
	);
	process.exit(1);
}
...
console.log(
	`API keys available: ${API_KEYS.length} (${API_KEYS.length * 25} total requests)\n`,
);

if (allFetches.length > API_KEYS.length * 25) {
	console.error(
		`Not enough API quota: need ${allFetches.length}, have ${API_KEYS.length * 25}`,
	);
	process.exit(1);
}
...
for (const { ticker, fy, fq } of allFetches) {
	...
	try {
		...
		if (transcript) {
			console.log(
				`  [${success + failed + 1}/${allFetches.length}] ${label}: ${transcript.sections.length} sections cached`,
			);
			success++;
		} else {
			console.log(
				`  [${success + failed + 1}/${allFetches.length}] ${label}: no transcript available`,
			);
			failed++;
		}
	} catch (e) {
		console.error(
			`  [${success + failed + 1}/${allFetches.length}] ${label}: FAILED - ${e}`,
		);
		failed++;
