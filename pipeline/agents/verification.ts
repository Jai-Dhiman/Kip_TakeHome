...
import { type MetricMapping, resolveMetric } from "../data/metric-mappings";
...
export class VerificationEngine {
	...
	private async verifyGrowthRate(
		...
		if (claim.comparison_basis?.toLowerCase().includes("year")) {
		...
		const currentNorm =
			Math.abs(currentValue) > 1_000_000
				...
				: currentValue;
		const priorNorm =
			Math.abs(priorValue) > 1_000_000 ? priorValue / 1_000_000 : priorValue;
