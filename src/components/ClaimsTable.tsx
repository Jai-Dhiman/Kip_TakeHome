...
const columns = [
	columnHelper.accessor((row) => row.verification?.status ?? "unverifiable", {
		id: "verdict",
		header: "Verdict",
		cell: (info) => {
	...
	columnHelper.accessor((row) => row.verification?.actual_value ?? null, {
		...
		cell: (info) => {
			...
			return (
				<span className="text-ink-700 font-mono text-xs">{val.toFixed(2)}</span>
			);
		},
	}),
	columnHelper.accessor(
		(row) => row.verification?.deviation_percentage ?? null,
		{
			...
			cell: (info) => {
				...
				if (val === null)
					return (
						<span className="text-parchment-400 text-xs font-mono">--</span>
					);
				const absVal = Math.abs(val);
				const color =
					absVal > 10 ? "#B54A32" : absVal > 5 ? "#C48B20" : "#2D7A4F";
				return (
